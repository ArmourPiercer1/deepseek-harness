...
Codex 和 Claude Code subagent 提供方是各自独立的可选组合包。可以只添加其中一个包、在一条命令中同时添加两个包，也可以独立移除其中任何一个包：
...
成功的 pnpm 操作会修改磁盘上的 Profile manifest 与 Bundle 列表；正在运行的 Profile 保留其当前启动时的 Bundle 集合。添加、移除或更新 Bundle 后，需要重启该 Profile。该启动边界适用于 Bundle 成员资格，而对 Profile 或 home `cordis.patch.yml` 的普通编辑通过热加载生效。下次启动时，每个已安装的 Bundle 只注册其休眠的 Host 提供方；复制出来的 Preset 需要单独为新的 Agent 启用对应的工具行。[Codex 提供方 README](../../../packages/subagent/subagent-codex/README.zh.md) 与 [Claude Code 提供方 README](../../../packages/subagent/subagent-claude-code/README.zh.md) 负责可执行文件、认证、payload 与故障细节；[基础 Bundle 参考](../../../packages/bundle/base/README.zh.md) 负责默认依赖闭包。
...
解析器镜像与 settings 写入的决策见 [teammate Agent Note](../../../.agents/notes/implemented/feature/2026-08-19-dsh-teammate-cli.zh.md)。

## Web 别名

`dsh web` 是 `--profile web` 的硬编码别名；写在它之后的 flag 属于 web 应用，由组合包中的普通提供方解析。`--host` 和 `--port` 覆盖承载它们的那些行的组合取值，可重复的 `--trusted-host` 通过 `ctx.webRuntime.trustedHosts` 提供本次调用的 authority（部署表达式会拼接自己的 authority），`--no-open` 则禁用本次调用的默认浏览器跳转。客户端插件 HMR（热模块替换）接收器始终挂载，在单独运行的 `pnpm run dev:web` watcher 重建客户端 bundle 之前保持空闲。
...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`，本地启动时，仅在完整 Loader 树就绪后才打开该规范主机 URL。继承到的非空 `SSH_CONNECTION` 或 `SSH_TTY` 会抑制浏览器跳转，因为本地转发地址由 SSH 客户端或编辑器持有；主机 URL 仍会打印。CLI 目前有意不支持 `--host 0.0.0.0`，并会以用法错误退出。本地跳转之前，会打印 `dsh web: opening the default browser; pass --no-open to disable`；如果操作系统跳转失败，stderr 上的诊断信息会说明原因、保持服务器运行，并给出供手动访问的 URL。`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority。
...
新会话默认使用 `workspace-write` 权限预设。Bash 和文件系统修改仅限于会话 workspace 与平台临时根目录；读取和网络访问不受限制，而进程可见性取决于所选沙箱后端——bwrap 在私有 PID 命名空间中运行命令，从而隐藏宿主进程；Landlock 和 Seatbelt 保持宿主进程可见性不变。`DSH_PERMISSION_MODE` 更改进程后备值。General settings 中存储的权限影响后续 Web 会话，不改变已打开的会话。
