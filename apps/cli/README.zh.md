...
| `dsh teammate list\|add\|enable\|disable` | 管理 teammate 定义与按工作区的启用状态；不启动任何 profile。 |
...
层的确切优先级、flag、关闭行为、部署默认值、源码执行方式和 teammate 命令族，以 [CLI（命令行界面）行为参考](reference/README.zh.md)为准。
