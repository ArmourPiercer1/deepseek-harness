...
describe('the shipped Web composition', () => {
  ...
  it('supplies the five shipped presets, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['cordis', 'minimal', 'ptc', 'standard', 'team'])
...
describe('a composition that configures its own preset roots', () => {
  ...
  it('keeps configured roots alongside the always-prepended shipped root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['cordis', 'minimal', 'ptc', 'standard', 'team', 'team-spec'])
