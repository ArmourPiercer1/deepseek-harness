...
`shellDialect` 选择 shell 技术栈（默认 `bash`，`pwsh`）：它决定默认的 `shellPath`/`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经由共享的 `dsh-pwsh-local` resolver 使用 `-NoLogo -NoProfile`）以及启动约定。bash 方言通过环境变量安装提示符（`PS1` 外加以 OSC `133;D;` 结尾的 `PROMPT_COMMAND`）。pwsh 无法从环境变量安装提示符，因此后端通过会话写入 `prompt` 函数，并等待受控提示符真正可见——由于 pwsh 横幅到提示符的间隔可能超过静默上限，期间会循环进行后续发送——同时其环境去掉仅 bash 使用的标记并加入 `NO_COLOR`。首次发送还会前置共享的 `dsh-pwsh-local` 编码前言，在任何内容运行之前将 `[Console]::OutputEncoding` 与 `$OutputEncoding` 固定为 UTF-8：会话解码路径按 UTF-8 读取 PTY 字节，而未经固定的控制台在非 ASCII 输出时写入其宿主机代码页。两个方言发出相同的以 BEL 结尾的 OSC 标记，因此就绪机制与消费方都与方言无关。
...
- 精确 stdin 等待检测取决于已挂载的进程管理提供方；无法证明该状态的提供方使用提示符标记和静默／超时就绪机制。Windows 就是这样一种提供方：shell pid 是伪前台进程组，且没有精确 stdin 等待层级，因此无标记的子进程会在静默上限处结算。
- pwsh 启动引导通过 `[Console]::` 写入（UTF-8 编码固定与 prompt 函数），Windows ACL 沙箱的只读模式（ConstrainedLanguage）可能拒绝这些写入。shell 仍可通过受控的可打印提示符与静默层级结算，但标记就绪不可用，非 ASCII 输出可能遵循宿主机代码页。
