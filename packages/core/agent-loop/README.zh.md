...
每次提供方调用成功结束时，都会恰好追加一个 `assistant/message` 完成锚点，包括无内容调用和以 `max-tokens` 结束的调用。该锚点原样记录组装后的内容，在 `sourceEventSeqs` 中列出确切的分片 seq（流没有分片时为 `[]`），并在用量可用时包含用量；空内容不会进入派生消息历史。中断流式传输的轮次取消，在已有非空文本或推理内容到达用户时，也会追加一个 `interrupted: true` 锚点。该锚点引用这些分片 seq，并将已渲染前缀放入派生消息历史，因此下一个请求会包含用户已看到的内容。未分发的工具调用会被省略，空流或仅包含工具的流不产生锚点；提供方失败仍不会提交任何 assistant 内容（参见[决策](../../../.agents/notes/implemented/architecture/2026-08-10-cancelled-stream-prefix-finalize.zh.md)）。
