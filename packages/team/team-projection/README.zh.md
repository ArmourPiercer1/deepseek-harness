...
| `onChanged(listener)` | 每个 leader 已提交状态恰好一次全量快照发布（last-wins）；触发源为团队事件族、leader `delegate_to_teammate`/结算通知、团队 child 创建/销毁、`agent/status`（仅重算本进程已折叠过快照的 leader）。落在进行中之折叠读日志之后的触发恰好重挂一次后续发布，任何已提交触发不丢失；折叠在读日志前夭折（store 已消失）则不重挂。 |

<a id="team-ness-gate"></a>
...
- `members` 为启用名册 ∪ 语料绑定：从未绑定的 teammate 发布 `status: 'unbound'` 行；已绑定但名册已删的成员以标签派生显示名保留；leader 行锚定 `sessionIds: [leaderSessionId]` 且永不为 `settled`，其 `memberId` 取名册 leader 条目的 id，名册无 leader 时回落 `'leader'` —— 若非 leader 名册条目已占用 `'leader'` 则改用 leader 会话 id。
