...
- **无持久 shell** — 每次调用都启动全新的 `pwsh -Command`；持久 shell 的对位包是 [`@deepseek-ai/dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.zh.md)，它在 Windows（ConPTY）与带 pwsh 的 POSIX 主机上让一个 owner 作用域的 pwsh 跨调用存活。
