...
bwrap profile 组合只读宿主根、全新的 `/dev` 与私有 PID 的 `/proc`。命令可管理其后代进程，但看不到宿主进程；隐藏宿主 `/proc/<pid>` 条目可防止 `root` 和 `fd` 等魔法链接绕过其挂载。`workspace-write` 另加临时 `/tmp` 和可写工作区绑定。[私有 PID Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-06-bwrap-private-pid-namespace.zh.md) 记录了该边界。
