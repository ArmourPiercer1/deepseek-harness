...
export interface TestSessionRemote {
  ...
  team: {
    projection(request: TeamProjectionRequest, signal?: AbortSignal): Promise<RemoteResult<TeamProjectionValue>>
  }
}
...
function installControllers(
  ...
  if (ctx.get('typert') === undefined) {
    // SessionTeamController's fiber waits on the Typert registry; a stub
    // activates it — no carrier runs in a unit harness.
    ctx.provide('typert', {})
  }
...
export function createSessionTestRemote(
  ...
  return {
    ...
    team: {
      projection: (request, signal = new AbortController().signal) => remoteResult(
        () => (ctx.get('sessionTeamController') as SessionTeamController | undefined)?.projection(request, signal)
          ?? Promise.reject(new TypertRemoteFailure({
