...
import type { SessionControlFrame, TeamProjectionValue } from '@deepseek-ai/dsh-api-session-controller/types'
...
describe('team mirror', () => {
  ...
  const teamFrame = (sessionId: SessionId, team: TeamView): SessionControlFrame =>
    ({ type: 'team', sessionId, team })
  ...
  it('merges whole snapshots last-wins, keyed by the view leader, never the frame session', () => {
    ...
    manager.handleControlFrame(teamFrame(MEMBER, first))
    ...
    manager.handleControlFrame(teamFrame(LEADER, second))
  ...
  it('keeps the record reference stable until a frame or a cold pull moves the mirror', async () => {
    ...
    api.onList = () => Promise.resolve(ok({ items: [summary(S1)] as never[] }))
    const manager = new SessionManager(fakeRemote(api))
    manager.handleControlFrame(teamFrame(LEADER, teamView(LEADER)))
    ...
    manager.handleControlFrame(teamFrame(LEADER, teamView(LEADER, { rosterMemberCount: 4 })))
    expect(manager.getTeamMirror()).not.toBe(before)
  })

  it('re-baselines the whole mirror on the baseline frame, which the Host re-pushes per team', () => {
    const manager = new SessionManager(fakeRemote(new FakeApiClient()))
    const own = teamView(LEADER)
    // A different leader's team whose member row binds a different session:
    // both entries live in the same mirror record.
    const other = teamView('other-leader' as SessionId, {
      members: [
        ...
        { memberId: 'other-mate', name: 'other-mate', role: 'teammate', sessionIds: ['other-mate'], status: 'running', pendingControlCount: 0 },
      ],
    })
    manager.handleControlFrame(teamFrame(LEADER, own))
    manager.handleControlFrame(teamFrame('other-leader' as SessionId, other))
    expect(manager.getTeamMirror()[LEADER]).toBe(own)
    expect(manager.getTeamMirror()['other-leader' as SessionId]).toBe(other)
    // A new generation's baseline clears every entry; the Host's
    // post-baseline `team` pushes re-establish whichever teams survive.
  ...
  it('drops the team a removed session led, while a removed member keeps the leader view', () => {
    ...
    manager.handleControlFrame(teamFrame(LEADER, view))
    manager.handleSessionRemoved(MEMBER)
    expect(manager.getTeamMirror()[LEADER]).toBe(view)
    manager.handleSessionRemoved(LEADER)
  ...
  it('cold-pulls single-flight and anchors the response leader, not the requested session', async () => {
    const api = new FakeApiClient()
    const gate = deferred<RemoteResult<TeamProjectionValue>>()
    ...
    gate.resolve(remoteOk(teamView(LEADER)))
    ...
    // snapshot wins.
    api.onTeamProjection = () => Promise.resolve(remoteOk(teamView(LEADER, { rosterMemberCount: 9 })))
  ...
  it('keeps mirror absence on business rejection, transport failure, and a page response', async () => {
    ...
    api.onTeamProjection = () => Promise.resolve(remoteOk<TeamMessagePage>({
  ...
  it('propagates the business rejection loud without a snapshot fallback', async () => {
    const api = new FakeApiClient()
    api.onTeamProjection = payload => Promise.resolve(remoteErr<TeamProjectionValue>({
      code: 'team-anchor-unknown',
      message: 'anchor named no folded message',
      details: { leaderSessionId: (payload as { leaderSessionId: SessionId }).leaderSessionId },
    }))
    const manager = new SessionManager(fakeRemote(api))
    const view = teamView(LEADER)
    manager.handleControlFrame(teamFrame(LEADER, view))
    ...
    expect(result).toEqual({
      ...
      error: {
        code: 'team-anchor-unknown',
        message: 'anchor named no folded message',
        details: { leaderSessionId: LEADER },
      },
    })
  ...
  it('answers a snapshot crossed to a page request as a loud error', async () => {
    const api = new FakeApiClient()
    api.onTeamProjection = () => Promise.resolve(remoteOk(teamView(LEADER)))
    const manager = new SessionManager(fakeRemote(api))
    const result = await manager.pageTeamMessagesBefore(LEADER, { at: 2, sessionId: MEMBER, seq: 1 })
    expect(result).toEqual({
      ok: false,
