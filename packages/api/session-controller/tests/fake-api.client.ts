...
export class FakeApiClient {
  ...
  /** Team projection (default: the team-ness gate rejects the unknown leader, so the mirror stays absent). */
  onTeamProjection: (payload: unknown) => Promise<RemoteResult<TeamProjectionValue>> =
    payload => Promise.resolve(remoteErr({
      code: 'team-leader-unknown',
      message: 'the session is not a team leader',
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      team: {
        projection: payload => this.record('team.projection', payload, this.onTeamProjection(payload)),
      },
      workspace: {
