...
export class SessionManager {
   ...
   * leader session, last-wins from the control stream's `team` frames. The key
   * is the view's own `leaderSessionId`, never the frame's sessionId — a
   * bound-teammate baseline frame carries the teammate's sessionId but the
   * anchored leader's view. An absent key means "no team known for that
   * leader"; absence, never a sentinel. The record reference is stable until
   * a frame or a cold pull moves the mirror, so per-leader selector consumers
  ...
  refreshTeam(sessionId: SessionId): Promise<void> {
    ...
    const operation = (async () => {
      try {
        const result = toSessionResult(await this.remote.team.projection({ leaderSessionId: sessionId }))
  ...
  async pageTeamMessagesBefore(
    ...
    try {
      const result = toSessionResult(await this.remote.team.projection({ leaderSessionId, messagesBefore: anchor, limit }))
      ...
      if (!('kind' in result.value)) {
        return transportResult(new Error('team.projection answered a page request with a snapshot'))
      }
  ...
  */
  ...
  private replaceControlBaseline(baseline: SessionControlBaseline): void {
    ...
    // The mirror rides no baseline field: the Host pushes one `team` frame
    // per live team right after the baseline, so this generation's views
    // cannot survive a reconnect un-pushed.
    this.teamByLeader = {}
  ...
  handleSessionRemoved(sessionId: SessionId): void {
    ...
    // fold); drop the leader key, never a member's view (its leader's fold
    // re-publishes without the member).
    if (this.teamByLeader[sessionId] !== undefined) {
      const { [sessionId]: _team, ...rest } = this.teamByLeader
