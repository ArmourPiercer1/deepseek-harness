...
export class SessionControlController {
  ...
  constructor(private readonly ctx: Context) {
    ...
    // Team projection change feed — whole-snapshot `team` pushes (the jobs
    // posture: one authoritative value per leader, last-wins, no deltas). The
    // service folds from durable logs, so the feed activates only when the
    // projection service is composed and unwinds with this controller's fiber;
    // a composition without it simply never pushes.
    ctx.inject(['teamProjection'], (teamCtx) => {
      teamCtx.teamProjection.onChanged((leaderSessionId, team) => {
        this.broadcast({ type: 'team', sessionId: leaderSessionId, team })
      })
    })
  ...
  async *control(signal: AbortSignal): AsyncIterable<SessionControlFrame> {
    ...
    try {
      ...
      // that fails the service's team-ness gate (an ordinary non-team session)
      // is rejected and stays absent, which is how a client reads "no team".
      // Frames push straight into this consumer's queue, not the broadcast:
      // the stream is still opening. A settle is not awaited — the queue
      // preserves arrival order and the generator drains it either way.
      ...
      if (teamProjection !== undefined) {
        for (const session of this.ctx.sessions.list()) {
          void teamProjection.get(session.id).then((team) => {
            queue.push({ type: 'team', sessionId: session.id, team })
          }, () => {
            // A session the gate rejects (ordinary session, unknown, or an
            // unreadable corpus) has no baseline; absence semantics apply.
          })
        }
      }
