...
export type SessionControlFrame =
  ...
  // The mirror key is the view's own leaderSessionId, never this sessionId: a
  // bound-teammate baseline frame carries the teammate's sessionId with the
  // anchored leader's view.
  | { readonly type: 'team'; readonly sessionId: SessionId; readonly team: TeamView }
  | ({ readonly type: 'projection' } & SessionProjectionUpdate)

/**
 * Wire bound on one team message page: `limit` is an integer in
 * `[1, TEAM_MESSAGE_CAP]`, host-validated in the `team.projection` Remote.
 */
