...
- **待处理问题无法在宿主重启后保留**：注册表持有等待中的工具调用自身的 `resolve`/`reject`，因此它只是宿主进程内存。`events.mux` 在每次重新打开时重放每个仍未处理的问题，可覆盖浏览器重新加载或重连；宿主重启会连同等待中的轮次一并带走，重新打开的 Session 不再为那个问题提供 composer。跨宿主重启恢复它需要一条持久的待处理交互记录，已列入暂缓。
