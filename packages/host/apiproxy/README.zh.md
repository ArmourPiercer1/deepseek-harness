...
团队域将宿主侧只读的[团队投影](../../team/team-projection/README.zh.md)暴露到线路上。当组合中有 `ctx.teamProjection` 时，网关订阅它的变更订阅，并在每次发布后为 leader 会话广播一份完整的 `session/team` 快照——与 `session/jobs` 同姿态：每个 leader 一个权威值，last-wins，无增量。流打开时做一次 baseline 遍历，对每个活体会话尝试投影：通过团队性门槛的会话向打开中的流排入一帧 `session/team`；被门槛拒绝的会话（普通会话、未知 id 或不可读语料）没有帧——缺席即客户端的"无团队"语义。一元 `team.projection` 按需应答：`{ leaderSessionId }` 返回全量快照；加上 `messagesBefore`（快照消息序中的 `{ at, sessionId, seq }` 锚点）与可选 `limit`（`[1, 500]`）则返回严格早于该锚点的 `TeamMessagePage`；响应值先为页（带显式 `kind`），后为无 kind 的快照。没有该服务的网关应答 `team-unavailable`；门槛失败应答 `team-leader-unknown`；折叠不服务的锚点应答 `team-anchor-unknown`。
