...
**Consumer**——读取 `ctx.permission`（来自 `@deepseek-ai/dsh-permission`），不提供服务。它松散地置于组合中并解析宿主权限引擎。团队插件是另一个消费者，改为将 `ask` 路由到 leader 会合点。
...
- **`ask` 路径依赖审批 seam**——守卫将 `ask` 路由到审批 seam；团队插件改为将 `ask` 路由到 leader 会合点，目前还没有任何会话日志组合测试证明这两条路径。
