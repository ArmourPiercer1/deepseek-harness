...
Workspace 列表基线就绪后，浏览器持久化的展开状态与 Session 顺序记录只保留当前 Workspace id、Ungrouped 与单列表记账。真实 Workspace 从 `WorkspaceView.sessionIds` 初始化，Ungrouped 与跨 Workspace 单列表从最近更新时间顺序初始化。共享侧边栏投影会隐藏持久化 Session 摘要中带有 `origin: 'subagent'` 的行；每个可见普通行都会在经不间断的 subagent 谱系可达的任一后代运行时继承蓝色活动指示器。Teammate 子会话以父级 subagent 目录为准识别，而非摘要的 origin：目录中任一 label 带 `team:` 前缀的 `child` 条目都会把该会话从所有顶层列表界面（分组、单列表、搜索）中隐藏，即使其摘要不携带 origin 同样如此。普通 subagent 子会话、无 label 的一次性子会话与目录诊断行保持既有处理不变。会话行还会在且仅在该会话以 leader 键存在于 sessions 服务提供的团队投影镜像时，渲染一枚团队徽标——成员图标加成员数；计数为花名册定义的成员数（含 leader），从镜像原样取得。仅作为已绑定成员出现在镜像团队中的会话、以及镜像中缺席的会话都不渲染徽标；sessions 面不具备团队能力时降级为静态空镜像，因此没有任何会话带徽标。
