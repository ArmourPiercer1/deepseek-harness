...
Once the Workspace list baseline is ready, browser-persisted expansion and Session-order records retain only current Workspace ids plus Ungrouped and the flat-list account. Real Workspaces initialize from `WorkspaceView.sessionIds`, while Ungrouped and the cross-Workspace flat list initialize from recency. The shared sidebar projection hides rows whose durable Session summary has `origin: 'subagent'`, and each visible ordinary row inherits the blue activity indicator while any descendant reached through uninterrupted subagent-origin lineage is running. Teammate children are identified by their parent's subagent catalog instead of the summary origin: any `child` catalog entry whose label carries the `team:` prefix hides that session from every top-level list surface (grouped, flat, and search), even when its summary carries no origin. Ordinary subagent children, unlabeled one-shot children, and catalog diagnostic rows keep their existing treatment. A session row additionally renders a team badge — a member glyph and the member count — exactly when that session exists as a leader key in the team projection mirror the sessions service provides; the count is the roster-defined member count (leader included), taken verbatim from the mirror. Sessions that appear only as bound members of a mirrored team, and sessions absent from the mirror, render no badge; a sessions face without the team capability degrades to a static empty mirror, so no session badges.
