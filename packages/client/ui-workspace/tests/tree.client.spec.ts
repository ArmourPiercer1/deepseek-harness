...
import type { SessionListState, SessionSummary, SubagentCatalogSnapshot } from '@deepseek-ai/dsh-api-session-controller/client'
...
const teamWireView = (leader: string, rosterMemberCount: number, memberIds: readonly string[] = []): TeamMirror[SessionId] => ({
  teamId: leader,
...
describe('D4 teammate hiding', () => {
  ...
  it('hides a teammate child from every list surface through its parent catalog label', () => {
    ...
    const groups = deriveGroups(sessions, [workspace('first', ['leader', 'mate'])], noArchive, noAttention, view(['first']))
    expect(groups[0]!.sessions.map(node => node.id)).toEqual([leader.id])
    expect(groups[0]!.sessionCount).toBe(1)
    expect(deriveFlat(sessions, noArchive, noAttention).map(node => node.id)).toEqual([leader.id])
    // Neither a title match nor a backend content hit surfaces the teammate.
    const search = deriveSearchResults(
      sessions, [workspace('first', ['leader', 'mate'])], 'mate', noArchive, noAttention,
  ...
  it('hides a catalog-labeled teammate even without the subagent origin flag', () => {
    ...
    expect(deriveFlat(sessions, noArchive, noAttention).map(node => node.id)).toEqual([leader.id])
    const strayGroups = deriveGroups(sessions, [], noArchive, noAttention, view([UNGROUPED_KEY]))
  ...
  it('keeps ordinary subagent, unlabeled one-shot, and diagnostic children on their existing treatment', () => {
    ...
    const groups = deriveGroups(
      sessions, [workspace('first', ['leader', 'regular', 'one-shot', 'broken'])], noArchive, noAttention, view(['first']),
    )
    expect(groups[0]!.sessions.map(node => node.id)).toEqual([leader.id, oneShot.id, broken.id])
    expect(deriveFlat(sessions, noArchive, noAttention).map(node => node.id)).toEqual([broken.id, oneShot.id, leader.id])
    expect(deriveSearchResults(
      sessions, [workspace('first', ['leader', 'regular', 'one-shot', 'broken'])], 'one-shot', noArchive, noAttention,
