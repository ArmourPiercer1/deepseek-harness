...
**获取可用模型**会针对表单**当前显示**的端点调用 `llm.discoverModels`，包括已修改但尚未保存的 API 地址和已键入但尚未存储的密钥，因此新增一个提供方是一趟走完，而不是「先保存再回来」。回复会打开一个选择框而不是直接写入：已配置过的候选默认不勾选，因此采纳一次选择绝不会覆盖用户已更正的容量。选择框按各 id 的前导 token 分组——`claude-3-5-sonnet` 归入 Claude、`gpt-4o` 归入 GPT、`gemini-2.0-flash` 归入 Gemini——让同时服务各家目录的网关读起来就像它实际包含的家族；一个**全选**开关选取整个列表，全选状态下显示为其反义（取消全选）。无法被询问的提供方只是绕路而非死路——适配器自己的消息会显示在各行旁边，而这些行仍可手工编辑。
