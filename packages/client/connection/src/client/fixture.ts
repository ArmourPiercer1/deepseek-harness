...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  /** Canonical fixture implementation of the generated team projection Remote contract. */
  const teamRemotes = {
    ...
    projection(request: { readonly leaderSessionId: SessionId }): RpcResult<unknown> {
      return {
        ok: false,
        error: {
  ...
  const rpc: ClientConnectionRpc = {
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'team/projection': return Promise.resolve(
          teamRemotes.projection(request as { leaderSessionId: SessionId }),
        )
