...
一个带版本的 YAML 文档，每个键空间一个 section，除此之外别无他物：
...
该文档只存放凭据，因此任何偏离都是拒绝，而不是跳过某个条目——被静默忽略的键读起来就是「我存进去的密钥没有生效」。非 mapping 的根、未知的顶层键、在其键空间中不可寻址的键、类型错误的值、空字符串、未知的 record tag 或字段、重复键以及格式错误的 YAML 全部失败：启动时明确报错，运行期热重载则告警并保留最后可用快照。

一个 `grant` payload 必须经受住 JSON 往返，且两个方向都强制执行。YAML 能写出 JSON 没有表示的值——`.inf`、别名环——而属主还可能递上一个 `Date` 或 `bigint`；无论哪种情况，存储都会拒绝，而不是存下一个无法按写入原样读回的东西。

预发布版本的布局是没有 `version` 的扁平 mapping。一次启动若能精确识别它——可寻址的名字位于非空字符串标量之上、没有 directive——就在写锁下就地升级该文档：原始行原样嵌套到 `refs:` 之下，因此值、注释与拼写逐字节保留。其他任何扁平形状都会按名字被拒绝，并附上条目数与唯一需要的那处编辑（`version: 1`，嵌套到 `refs:` 之下）——绝不会被读成空存储，那只会让问题在第一次请求时以认证失败的形式冒出来，而不是在加载时。运行期热重载从不迁移：运行中途恢复的扁平文档会保留最后可用快照，直到下一次启动。
