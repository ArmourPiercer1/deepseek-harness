...
| 工具 | 参数 | 行为 |
|---|---|---|
| `web_search` | `queries`（必填 string[]） | 用于发现信息。返回可选答案与来源 URL。并发执行 1 到 `searchMaxQueries` 个不同搜索，先按轮询顺序合并其来源，再应用合并后的 `searchMaxResults` 上限。单元素数组执行一次搜索。完全重复的查询只执行一次。任一搜索失败会中止剩余批量，该批量在调用返回错误前落定。两个上限都不面向模型。 |
...
| `searchMaxResults` | `8` | 一次 `web_search` 调用返回的来源数量上限（seam 截断每个提供方列表；工具也对合并的多查询列表设限）。 |
| `searchMaxQueries` | `4` | 一次 `web_search` 调用接受的查询数量上限。该配置值会出现在其提示词指引与 schema 描述中。 |
...
每个通过配置启用的工具都会为每次请求增加固定的指引 token 开销，即使限制隐藏了其 schema。切换抓取或更改 `searchMaxQueries` 会改变搜索指引；切换抓取还会注册或移除抓取区段。

#### KV Cache 影响

只要启用工具、scope 与指引文本不变，前缀就保持稳定。配置启用状态——包括切换抓取的搜索指引分支——、更改 `searchMaxQueries` 或插件生命周期可能使从第一个变化的提示词区段起的复用失效；scope schema 限制不会移除该区段。
...
每次请求都会为解析后的 `searchMaxQueries` 产生固定的 schema token 开销；通过配置禁用会同时移除 schema 与指引，scope 限制只移除 schema。

#### KV Cache 影响

只要定义、解析后的查询上限与可见性不变，前缀就保持稳定。配置启用状态、更改 `searchMaxQueries`、插件生命周期或 scope 限制可能使从第一个变化的 schema token 起的复用失效。

### 搜索结果

#### 模型看到的内容

可选的提供方答案之后是 `Sources:`，再跟随内容取决于数据且格式严格为 `- [<title-or-url>](<url>)` 的行，并可添加后缀 ` — <snippet> (<publishedAt>)`。多查询调用每个精确查询字符串只执行一次，并保留其首次出现的位置；以 markdown 标题标注每个提供方答案的来源查询，按 URL 去重来源，并在推进到下一排名之前从每个查询的每个排名各取一个来源。既无答案也无来源时，结果显示 `No results found.`。列表被截断至上限时会添加 `(Showing the first <count> sources. Refine the query for more.)`；每个结果都以 `Cite the relevant URLs above as markdown links in your answer.` 结尾。

#### Token 影响

数据相关结果会重复发送直到压缩（compaction）；查询扇出由 `searchMaxQueries` 限制，来源数量由 `searchMaxResults` 限制。
...
多查询调用中任一查询失败时，`web_search` 中止其余搜索，等待所有已开始的搜索落定，丢弃成功的结果，并对第一个失败返回 `Error: <message>`。
...
Schema 验证在执行前拒绝缺失或非数组的 `queries` 字段以及非字符串数组元素。值错误精确地变为 `Error: queries must contain at least one query`、配置上限为一时为 `Error: queries must contain at most 1 query`、更大上限时为 `Error: queries must contain at most <count> queries`、`Error: each query must be a non-empty string` 或 `Error: url must be a non-empty string`。
...
- **没有批级原生搜索计数器**：`searchMaxQueries` 限制 `ctx.web.search` 调用次数，但提供方可能在每次调用内部执行多次原生搜索。例如，配置了 `maxUses` 的模型驱动提供方最多可允许 `searchMaxQueries × maxUses` 次原生搜索；`searchMaxResults` 只限制返回给调用方的合并来源。由于通用 seam 不了解提供方内部的搜索单位，部署通过这些相互独立的消费者与提供方设置控制成本。
