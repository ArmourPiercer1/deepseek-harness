...
| `ctx.authorization` | `seam` | [`authorization`](../packages/credentials/authorization) | - | [`llm-pi-ai`](../packages/llm/llm-pi-ai) | - | 授权流由知道如何获取凭据的插件注册，并以写入的记录作为键；该 seam 拥有对话以及每键一次尝试的生命周期，从不涉及协议。 |
...
| `ctx.fileReferences` | `seam` | [`file-reference`](../packages/context/file-reference) | [`file-reference-local`](../packages/context/file-reference-local) | - | - | 该接口通过其一元 Remote 契约，返回所寻址 Agent cwd 内仅含路径的补全候选；提供方拥有命名空间访问与排序，且不读取文件内容。 |
...
| `ctx.agentTeams` | `core` | `agent-team` | - | `tool-agent-team` | - | 拥有隐含根名册、持久的同伴邮箱、共享任务 DAG 与可延续子级的生命周期；tool-agent-team 提供限定作用域的模型策略与控制。 |
